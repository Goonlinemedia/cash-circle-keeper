...
import { ArrowDownToLine, ArrowUpFromLine, Wallet, Users, TrendingUp, ArrowRight } from "lucide-react";
    ...
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="rounded-2xl bg-primary text-primary-foreground p-6 md:p-8 relative overflow-hidden">
        ...
        <div className="relative">
          <div className="text-sm/none text-primary-foreground/70 uppercase tracking-wider">Cash balance</div>
          <div className="font-display text-4xl md:text-5xl font-bold mt-2">{formatNaira(balance)}</div>
          <div className="mt-4 flex flex-wrap gap-2">
            <Button asChild size="sm" variant="secondary" className="bg-accent text-accent-foreground hover:bg-accent/90">
              <Link to="/collections"><ArrowDownToLine className="size-4" /> Record collection</Link>
            </Button>
            <Button asChild size="sm" variant="outline" className="bg-transparent border-primary-foreground/30 text-primary-foreground hover:bg-primary-foreground/10 hover:text-primary-foreground">
              <Link to="/withdrawals"><ArrowUpFromLine className="size-4" /> Record withdrawal</Link>
            </Button>
          </div>
        </div>
      </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button asChild variant="ghost" size="sm">
            <Link to="/transactions">View all <ArrowRight className="size-4" /></Link>
          </Button>
        </CardHeader>
        <CardContent>
          {recent.length === 0 ? (
            ...
            <ul className="divide-y">
              {recent.map((t) => {
                ...
                const label = t.type === "IN"
                  ? (customer?.name ?? "Cash in")
                  : (t.withdrawalType ?? "Withdrawal");
                return (
                  <li key={t.id} className="flex items-center gap-3 py-3">
                    <div className={`size-9 rounded-full grid place-items-center shrink-0 ${t.type === "IN" ? "bg-success/15 text-success" : "bg-destructive/15 text-destructive"}`}>
                      {t.type === "IN" ? <ArrowDownToLine className="size-4" /> : <ArrowUpFromLine className="size-4" />}
                    </div>
                    ...
                    <div className={`font-semibold tabular-nums ${t.type === "IN" ? "text-success" : "text-destructive"}`}>
        ...
        <CardContent>
          {activeCustomers.length === 0 ? (
            <div className="text-sm text-muted-foreground py-6 text-center">
              No active customers yet. <Link to="/customers" className="text-primary underline">Add one</Link>.
            </div>
          ) : (
                ...
                .map((c) => (
                  <div key={c.id} className="flex items-center justify-between rounded-md border bg-card px-3 py-2">
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground">{formatNaira(c.contributionAmount)} · {c.frequency}</div>
                    </div>
                    <Badge variant="outline" className="border-accent/50 text-accent-foreground bg-accent/15">Pending</Badge>
                  </div>
                ))}
              {pending === 0 && (
      ...
      <CardContent className="pt-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className={`mt-2 font-display text-2xl font-bold tabular-nums ${toneCls}`}>{value}</div>
